 ...
 */
		...
		for (int i = 0; i < list.size(); i++) {
			Object data = list.get(i);
			if (data instanceof String && !NumberUtil.isNumber((String)data)) { // 如果是字符串，并且字符串不是数字
				exprBuilder.append("\"").append(data).append("\"");
			} else {
				exprBuilder.append(data);
			}
			if (i < list.size() - 1) {
		...
		Object obj=context.evalExpr(expr);
