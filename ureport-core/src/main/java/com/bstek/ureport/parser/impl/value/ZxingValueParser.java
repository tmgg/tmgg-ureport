...
public class ZxingValueParser extends ValueParser {
	...
	public void convert(Object o, Element el) {
		ZxingValue value= (ZxingValue) o;
		el.addAttribute("source", value.getSource().name());
		el.addAttribute("width", String.valueOf(value.getWidth()));
		el.addAttribute("height", String.valueOf(value.getHeight()));
		el.addAttribute("format", value.getFormat());
		el.addAttribute("category", value.getCategory().name());
		el.addAttribute("code-display",String.valueOf( value.isCodeDisplay()));


		Element child = DocumentHelper.createElement(value.getSource().name());
		child.setText(value.getValue());
		el.add(child);
	}
}
