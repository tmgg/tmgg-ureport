...
public class Cell implements ReportCell {
    ...
    public void doFormat() {
        ...
        if (data instanceof String && StringUtils.isBlank((CharSequence) data)) {
            return;
        }

        String format = cellStyle.getFormat();
        if (customCellStyle != null && StringUtils.isNotBlank(customCellStyle.getFormat())) {
            format = customCellStyle.getFormat();
        }


        // add by jiangtao: 默认double不使用科学计数法
        if (StringUtils.isBlank(format) && data instanceof Double) {
            format = "###,###.##"; // 默认千分位，保留2位小数
        }
    ...
    public String toString() {
        return name + ", " + data + ", " + data.getClass().getSimpleName();
    }
}
